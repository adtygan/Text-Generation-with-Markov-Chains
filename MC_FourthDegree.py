...
for pos, word in enumerate(cleaned_text_words):
    if pos <= len(cleaned_text_words)-5:
        ...
        word5 = cleaned_text_words[pos+4]
        if (word, word2, word3, word4) in transition_map:
            transition_map[(word, word2, word3, word4)].append(word5)
        else:
            transition_map[(word, word2, word3, word4)] = [word5]

def generate_text(transition_map, no_of_words, seed1, seed2, seed3, seed4):
    generated_text = [seed1, seed2, seed3, seed4]
    curr_word1 = seed1
    curr_word2 = seed2
    curr_word3 = seed3
    curr_word4 = seed4
    for i in range(no_of_words-1):
        next_word = random.choice(transition_map[(curr_word1, curr_word2, curr_word3, curr_word4)])
        generated_text.append(next_word)
        curr_word1 = curr_word2
        curr_word2 = curr_word3
        curr_word3 = curr_word4
...
seed4 = cleaned_text_words[choice+3]
no_of_words = 200
result = generate_text(transition_map, no_of_words, seed1, seed2, seed3, seed4)
# Unpacking the list 
